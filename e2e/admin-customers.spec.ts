...
/** Tạo manager qua UI admin: chọn role + nhà hàng, trả email đã tạo. */
async function createManagerViaAdmin(page: Page, restaurantName: string, email: string) {
  ...
  // Điền thông tin
  await page.getByPlaceholder('Nhập họ và tên người dùng').fill('Manager E2E');
  ...
  await expect(page.getByText('Tạo nhân viên thành công')).toBeVisible({ timeout: 15_000 });
}
