...
async function seedRestaurants(): Promise<void> {
  ...
  await DB_Connection.Restaurant.insertMany([
    ...
    // Chủ test subscription (T7): 3 nhà hàng ở các trạng thái gói khác nhau
    {
      ...
      name: 'NhamNhi Sub Basic',
      email: 'sub.basic@nhamnhi.vn',
      status: 'active',
    ...
    },
    {
      ...
      ownerId: SEED_IDS.ownerSub,
      subscription: 'active',
      paidUntil: new Date(now.getTime() + 3 * 24 * 3600 * 1000),
      currentPlanKey: 'pro',
    },
    {
      _id: SEED_IDS.tenantSubLocked,
      name: 'NhamNhi Sub Miễn Phí',
      email: 'sub.free@nhamnhi.vn',
      status: 'active',
      ownerId: SEED_IDS.ownerSub,
      subscription: 'active',
      currentPlanKey: 'free',
